  ...
  return (
    <nav className="w-full bg-white border-b border-slate-200 overflow-x-auto scrollbar-none shadow-xs sticky top-[108px] z-40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex space-x-1.5 sm:space-x-2.5 py-2 min-w-max">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-sm font-bold transition-all duration-150 ${
                  isActive
                    ? 'bg-[#166534] text-white shadow-sm'
                    : 'text-slate-700 hover:text-[#0F2E53] hover:bg-slate-100'
                }`}
              >
